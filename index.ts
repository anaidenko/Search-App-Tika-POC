  ...
  return new Promise((resolve: (data: ExtractedData) => void, reject) => {
    tika.xhtml(filepath, (err, xhtml) => {
      if (err) reject(err);
        ...
        $body.find('p').filter((i, p) => ($(p).html() || '').trim() === '').remove()
        // $body.find('.page').filter((i, page) => page.children.length === 0).remove()
        
        let html = $body.html() || '';

        // if ($body.find('a').length === 0) {
        //     html = html.replace(/\b(https?:\/\/[^\b\s ]+)\b/g, '<a href="$1">$1</a>');
        // }
...
async function indexContent(content: ExtractedData, es_index: string, filepath: string) {
    ...
    client.index(
      {
        ...
        body: {
          ...
          attachment: {
            ...
            // language: 'en', // tika.xhtml doesn't detect document language, try tika.language instead
...
async function indexContentBySegments(
  ...
  return new Promise((resolve, reject) => {
    ...
    let pages = content.$html.find('.page').toArray().map(page => $(page).children().length === 0 ? '' : $.html(page)).filter(page => page !== '')
    // pages = pages.map(page => page.replace(/\n*(<p>)\n*/g, '\n$1').replace(/(<\/?(p|div)[^>]*>)/gi, "")); // strip html tags

    // const whitelistTags = "table|th|tr|td|b|i|u|strong|ul|ol|li|small|sub|sup|em|span|font|big|del|s|strike|tt|blockquote".split("|");
    // pages = pages.map(page => page.replace(/\n*(<p>)\n*/g, '\n$1').replace(new RegExp('<(?!\/?(' + whitelistTags.join('|') + '))[^>]+>/g', 'gi'), "")); // strip html tags except tables

    let tables = content.$html.find('table').toArray().map(table => ({
      id: ++tableCount,
      content: $.html(table)
    }))
    ...
    const doc = {
      ...
      pages: pages.map(page => {
        ...
        let paragraphs3 = page.split(/<p[^>]*>/gi).map(page => page.replace(/<\/?(p|div)[^>]*>/gi, '')).filter(p => p !== '');
        return {
          id: ++pageCount,
          content: page,
          paragraphs: paragraphs3.map(p => {
            p = p.trim()
            const sentences3 = p.replace(/\n+/g, "").replace(/([.|!|?]+)/g, '$1\n').split('\n');
            return {
              id: ++paragraphCount,
              content: p,
              sentences: sentences3.map(sentence => {
